...
If the native `ios/` and `android/` directories in your project are generated by `expo prebuild` (Continuous Native Generation), skip the "Setting up iOS" and "Setting up Android" sections and go straight to the [Expo setup guide](./docs/expo.md).
...
If the native `ios/` and `android/` directories in your project are generated by `expo prebuild` (Continuous Native Generation), ignore the "Setting up iOS" and "Setting up Android" sections above and use a config plugin to update your native files. See [here](./docs/expo.md) for an **example** config plugin written for an expo app that uses this library.
